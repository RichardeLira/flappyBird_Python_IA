...
IMAGE_FUNDO = pygame.transform.scale2x(pygame.image.load(os.path.join('imgs', 'bg.png')))
IMAGENS_PASSARO = [
...
FONTE_GERACAO = pygame.font.SysFont('arial' , 40)
...
class Passaro: 
    ...
    def desenhar(self, tela):
        ...
        #se passaro cair, não bater a asa 
        if self.angulo <= -80:
            self.imagem = self.IMGS[1] 
...
def desenhar_tela(tela, passaros, canos, chao, pontos): 
    ...
    if ai_jogando: 
        texto = FONTE_GERACAO.render(f"Geração: {geracao}", 1, (255,255,255))     
